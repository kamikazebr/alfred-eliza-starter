...
import { mainnet, iotex, arbitrum, Chain, base } from "viem/chains";
...
describe("Wallet provider", () => {
    ...
    describe("Constructor", () => {
        ...
        it("sets default chain to base", () => {
            walletProvider = new WalletProvider(pk, mockCacheManager as any);

            expect(walletProvider.chains.base.id).toEqual(base.id);
            expect(walletProvider.getCurrentChain().id).toEqual(base.id);
        });
    ...
    describe("Balance", () => {
        ...
        it("should return null if chain is not added", async () => {
            const bal = await walletProvider.getWalletBalanceForChain("optimism");
            expect(bal).toBeNull();
        });
    });
    describe("Chain", () => {
        ...
        it("adds chain", () => {
            ...
            expect(initialChains.optimism).toBeUndefined();

            const optimism = WalletProvider.genChainFromName("optimism");
            walletProvider.addChain({ optimism });
            const newChains = walletProvider.chains;
            expect(newChains.optimism.id).toEqual(optimism.id);
        });
